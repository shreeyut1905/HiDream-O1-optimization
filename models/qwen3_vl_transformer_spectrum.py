...
from spectrum_utils import step_derivative_approximation , step_taylor_formula , set_method
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def _forward_generation(self, input_ids, position_ids, vinputs, timestep, token_types,
        ...
        # # if not hasattr(self, "_spectrum_initialized"):

        # #     self.cache_dic_cond, self.current_cond = cache_init(self)
        # #     self.cache_dic_uncond, self.current_uncond = cache_init(self)
        ...
        # #     set_method("spectrum")
        # #     self._spectrum_initialized = True
        # # actual_forward = True
        ...
        # #     if actual_forward:
        # #         # 1. Update activated steps BEFORE derivative approximation
        # #         if input_ids.shape[1] > 20:
        # #             self.current_cond["activated_steps"].append(self.current_cond["step"])
        # #         else:
        # #             self.current_uncond["activated_steps"].append(self.current_uncond["step"])

        # #         # 2. Run the heavy transformer model
        ...
        # #         # 3. Fit/Update the derivative approximation
        # #         if input_ids.shape[1] > 20:
        # #             step_derivative_approximation(
        # #                 cache_dic=self.cache_dic_cond,
        # #                 current=self.current_cond,
        # #                 feature=hidden_states
        # #             )
        # #         else:
        # #             step_derivative_approximation(
        # #                 cache_dic=self.cache_dic_uncond,
        # #                 current=self.current_uncond,
        # #                 feature=hidden_states
        # #             )

        # #     else:
        # #         # 4. SKIP THE DECODER! Use the Taylor expansion formula to predict the result
        # #         if input_ids.shape[1] > 20:
        # #             hidden_states = step_taylor_formula(
        # #                 cache_dic=self.cache_dic_cond, 
        # #                 current=self.current_cond
        # #             )
        # #         else:
        ...
        # # if input_ids.shape[1] > 20:
        # #     self.current_cond["step"] += 1
        # # else:
        # #     self.current_uncond["step"] += 1
        ...
        if not hasattr(self, "_spectrum_initialized"):
            ...
            # Isolate state variables PER CACHE to support interleaved CFG passes
            for cur_cache in [self.current_cond, self.current_uncond]:
                cur_cache["num_consecutive_cached_steps"] = 0
                cur_cache["warmup_steps"] = 100
                cur_cache["curr_ws"] = 2

            set_method("spectrum")
            self._spectrum_initialized = True

        # --- 1. Select the appropriate cache ---
        is_cond = input_ids.shape[1] > 20
        cur_cache = self.current_cond if is_cond else self.current_uncond
        cache_dic = self.cache_dic_cond if is_cond else self.cache_dic_uncond

        # --- 2. Determine if an actual forward pass is needed ---
        actual_forward = True
        
        # Check against the LOCAL cache step, not a global counter
        if cur_cache["step"] >= cur_cache["warmup_steps"]:
            actual_forward = (cur_cache["num_consecutive_cached_steps"] + 1) % int(cur_cache["curr_ws"]) == 0

        # --- 3. Update the consecutive skip tracker ---
        if actual_forward:
            cur_cache["num_consecutive_cached_steps"] = 0
        else:
            cur_cache["num_consecutive_cached_steps"] += 1

        # --- 4. Execute the Model OR the Taylor Formula ---
        if actual_forward:
            cur_cache["activated_steps"].append(cur_cache["step"])
            ...
            if use_flash_attn:
                ...
                # Proper non-flash attention fallback
                dtype = inputs_embeds.dtype
                min_val = torch.finfo(dtype).min
                attn_masks = []
                for b in range(batch_size):
                ...
                outputs = self.language_model(
                    input_ids=None, position_ids=position_ids, attention_mask=attention_mask_4d,
                    inputs_embeds=inputs_embeds, use_cache=False,
                    visual_pos_masks=visual_pos_masks, deepstack_visual_embeds=deepstack_visual_embeds,
                    return_mid_results_layers=return_mid_results_layers,
                )
                hidden_states = outputs.last_hidden_state
                if hasattr(outputs, 'mid_results'):
            ...
            # Fit the polynomial basis using the exact hidden states
            step_derivative_approximation(
                cache_dic=cache_dic, 
                current=cur_cache, 
                feature=hidden_states
            ...
            # Skip the heavy decoder layers and predict using Taylor approximation
            hidden_states = step_taylor_formula(
                cache_dic=cache_dic, 
                current=cur_cache
            )
        ...
        # --- 5. Increment the local step counter ---
        cur_cache["step"] += 1

        # --- 6. Apply final layer to get pixel predictions ---
        x_pred = self.final_layer2(hidden_states)  # [batch, total_seq_len, out_dim]
