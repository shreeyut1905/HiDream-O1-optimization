...
def generate_image(
    ...
    def forward_once(
        ...
        if actual_forward:
            ...
            if sample is samples[0]:
                ...
                # print(len(current["activated_steps"]))
                step_derivative_approximation(
                    cache_dic,
                    current,
                    x_pred.detach(),
            ...
            if sample is not samples[0]:
                raise RuntimeError(
                    "Spectrum prediction only supported for conditional branch"
                )

            x_pred = step_taylor_formula(
                cache_dic,
                current,
            )
            emb = None
        ...
        return (
            x_pred[0, sample["vinput_mask"][0]][:tgt_image_len].unsqueeze(0),
            emb,
            ds_emb,
        )
    ...
    spectrum_cfg = SpectrumConfig(len(sched.timesteps))

    cache_dic, current = cache_init(spectrum_cfg)


    cond_image_embeds = None
    cond_deepstack_image_embeds = None
    guidance_cutoff = 40   # disable CFG after step 20
    for step_idx, step_t in enumerate(tqdm.tqdm(sched.timesteps, desc="Generating")):
        ...
        if ref_patches is None:
            ...
            if step_idx < guidance_cutoff and len(samples) > 1:
                # x_pred_uncond = forward_once(samples[1], z.clone(), t_pixeldit)
                x_pred_uncond = forward_once(
                    samples[1],
                    z.clone(),
                    t_pixeldit,
                    current=current,
            ...
            if step_idx >= guidance_cutoff:
                active_samples = [samples[0]]  
